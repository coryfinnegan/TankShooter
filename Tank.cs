...
namespace Game1
{
    ...
    public class Tank : Microsoft.Xna.Framework.DrawableGameComponent
    {
        ...
        {
            texture = Game.Content.Load<Texture2D>(@"Textures\steel");
        ...
        public void setForward()
        {
            ...
            position.Z += (direction.Z / (tankSpeed));
                ...
                if (forwardVector.Y > 0.4f)
                {
                    position.X += (-direction.X / (tankSpeed));
                    position.Z += (-direction.Z / (tankSpeed));
                }
                if (forwardVector.Y > 1 && forwardVector.Y < 0.4f)
                {
                    position.X += (direction.X / tankSpeed);
                    position.Z += (direction.Z / tankSpeed);
                }
